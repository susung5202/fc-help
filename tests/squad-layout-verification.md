...
The corrected interaction shows translucent regions for all 27 field positions while a field player is dragged, with GK kept in its protected slot. The region under the pointer is highlighted, and the dragged card shows the hovered position and its OVR before release. Team-color preview requests are debounced, cancelled when the target changes, and cached by the projected squad. Drop-zone overlays are excluded from saved squad images.
