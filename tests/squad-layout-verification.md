...
The corrected interaction recognizes all 27 field positions while a field player is dragged, with GK kept in its protected slot. Only the region currently under the pointer is shown as a translucent highlight; the rest of the pitch stays clear. The dragged card shows the hovered position and its OVR before release. Team-color preview requests are debounced, cancelled when the target changes, and cached by the projected squad. Drop-zone overlays are excluded from saved squad images.
